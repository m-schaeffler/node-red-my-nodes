...
describe( 'hourmeter Node', function () {
  ...
  it('should have a set the counter value feature, off phase', function (done) {
  ...
  it('should work with objects', function (done) {
    this.timeout( 5000 );
    const reasons = ['on','off','query','reset'];
    var flow = [{ id: "n1", type: "hourmeter", topic:"zaehler", cycle:0, property: "payload.value", name: "test", wires: [["n2"],["n3"]] },
                { id: "n2", type: "helper" },
                { id: "n3", type: "helper" }];
    helper.load(node, flow, async function () {
      var n3 = helper.getNode("n3");
      ...
      n3.on("input", function (msg) {
        ...
        try {
          ...
          {
            msg.should.have.property('payload').which.is.approximately(500/3600000,50/3600000);
          }
        }
        catch(err) {
          done(err);
      ...
      try{
        ...
        c1.should.match( 0 );
        c2.should.match( 0 );
        n1.receive({ topic:"test", payload: { a: false, value: true } });
        await delay(50);
        c1.should.match( 1 );
        c2.should.match( 1 );
        n1.context().get("data").should.have.ValidData(true,true);
        ...
        c2.should.match( 1 );
        n1.context().get("data").should.have.ValidData(true);
        n1.receive({ topic:"test", payload: { a: true, value: false } });
        await delay(50);
        c1.should.match( 2 );
        c2.should.match( 2 );
        n1.context().get("data").should.have.ValidData(false);
